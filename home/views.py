...
def index(request):
    if request.method == 'POST':
        ...
        # render(request, 'home/faculty_dashboard.html', content)
                    ...
                    messages.error(
                        request,
                        'Sorry, either of your provided information is wrong.')
...
def driver_login(request):
    if request.method == 'POST':
        try:
            driver_data = Driver.objects.get(email=request.POST['email'])
            if driver_data:
                if driver_data.password == request.POST['password']:
                    request.session['driver'] = driver_data.pk
                    content = {'data': driver_data}
                    # messages.add_message(request, messages.INFO, 'Logged in Successfully')
                    messages.success(request, 'Logged in Successfully')
                    response = HttpResponseRedirect(reverse('home:dashboard'))
                    response.set_cookie('name', "Shubham")
                    return response
                    # return HttpResponseRedirect('/dashboard', content)
        # render(request, 'home/driver_dashboard.html', content)
                else:
                    content = {'data': driver_data}
                    # messages.add_message(request, messages.ERROR, 'No match Wrong Password.')
                    messages.error(
...
def parent_login(request):
    if request.method == 'POST':
        try:
            parent_data = Parent.objects.get(email=request.POST['email'])
            if parent_data:
                if parent_data.password == request.POST['password']:
                    request.session['parent'] = parent_data.pk
                    content = {'data': parent_data}
                    # messages.add_message(request, messages.INFO, 'Logged in Successfully')
                    messages.success(request, 'Logged in Successfully')
                    response = HttpResponseRedirect(reverse('home:dashboard'))
                    response.set_cookie('name', "Shubham")
                    return response
                    # return HttpResponseRedirect('/dashboard', content)
        # render(request, 'home/parent_dashboard.html', content)
                else:
                    content = {'data': parent_data}
                    # messages.add_message(request, messages.ERROR, 'No match Wrong Password.')
                    messages.error(
...
def student_login(request):
    if request.method == 'POST':
        try:
            studnt_data = Students.objects.get(
                roll_no=request.POST['univ_rlno'])
            if studnt_data:
...
def add_student(request):
    ...
    if request.method == 'POST':
        ...
        sectn_data = Sections.objects.get(name=faculty_data.section)
        stdnt = sectn_data.students_set.create(
